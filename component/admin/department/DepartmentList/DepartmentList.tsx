...
const DepartmentList = ({ onEdit }: DepartmentListProps) => {
  ...
  // NOTE: the header (and its search input) is rendered exactly once, at a
  // fixed spot in a single root element. Previously the "no results" case
  // returned a whole different element tree (a Fragment) than the normal
  // case (a <div>), which made React unmount/remount the input whenever a
  // search stopped matching anything — losing focus and cursor position
  // mid-type. Keeping one stable wrapper and branching only the content
  // *below* the header fixes that.
