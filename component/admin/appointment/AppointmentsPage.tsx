...
import useAcceptAppointment from "@/customHooks/AppointmentList/useAcceptAppointment";
...
export type TabFilter = "all" | "pending" | "accepted" | "cancelled";
export type ViewMode = "card" | "table";
 
export default function AppointmentsPage() {
  const { appointments, acceptedAppointments, loading, error } = useAppointmentList();
  const { handleCancel } = useCancelAppointment();
  const { handleAccept } = useAcceptAppointment();
  ...
  const filtered = useMemo(() => {
    const source = activeTab === "accepted" ? acceptedAppointments : appointments;
  ...
  return (
    <div style={styles.wrapper}>
      ...
      {!error && filtered.length > 0 && (
        viewMode === "card"
          ? <AppointmentCardGrid appointments={filtered} onCancel={openCancel} onAccept={handleAccept} />
          : <AppointmentTable appointments={filtered} onCancel={openCancel} onAccept={handleAccept} />
      )}
