...
export default function DashboardPage() {
  const { doctors, loading: doctorsLoading, total: totalDoctors } = useDoctorList();
  const { appointments, acceptedAppointments, loading: appointmentsLoading } = useAppointmentList();
  const { departments, loading: departmentsLoading } = useDepartmentList();
  ...
  // Sourced from the dedicated /admin/appointment/acceptedlist endpoint,
  // not a client-side status guess — this is the real accepted count.
  const accepted = acceptedAppointments;
